...
public class ExclusiveTimeOfFunctions {
	public int[] exclusiveTime(int n, List<String> logs) {
		...
		int[] startTime = new int[logs.size() / 2 + 1];
		int[] exclusiveTime = new int[logs.size() / 2 + 1];
		int i = 0;
		...
		for (String log : logs) {
			...
			if (str[1].equals("start")) {
				...
				int executeTime = timestamp - startTime[i] + 1 - exclusiveTime[i--];
				exclusiveTime[i] += executeTime + exclusiveTime[i + 1];
				result[Integer.parseInt(str[0])] += executeTime;
			}
		}
