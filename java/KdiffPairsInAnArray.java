...
public class KdiffPairsInAnArray {
	public static void main(String[] args) {
		...
		System.out.println(findPairs(nums, Integer.parseInt(args[args.length - 1])));
	}
	public static int findPairs(int[] nums, int k) {
		...
		while (rightIndex < nums.length) {
			...
			int diff = nums[rightIndex] - nums[leftIndex];
			if (diff == k) {
				count++;
				rightIndex++;
				leftIndex++;
				while (leftIndex < nums.length && nums[leftIndex - 1] == nums[leftIndex]) {
					leftIndex++;
				}
			} else if (diff > k) {
				leftIndex++;
