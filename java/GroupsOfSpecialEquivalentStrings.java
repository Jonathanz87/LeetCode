...
public class GroupsOfSpecialEquivalentStrings {
	public static void main(String[] args) {
		System.out.println(numSpecialEquivGroups(args));
	}
	...
	public static int numSpecialEquivGroups(String[] A) {
		...
		for (String s : A) {
			...
			int[] oddLetters = new int[26];
			int[] evenLetters = new int[26];
			int[] current = oddLetters;

			for (char c : s.toCharArray()) {
				current[c - 'a']++;
				current = current == oddLetters ? evenLetters : oddLetters;
			}

			for (int i = 0; i < oddLetters.length; i++) {
				builder.append((char)('a' + i)).append(oddLetters[i]);
			}
			for (int i = 0; i < evenLetters.length; i++) {
				builder.append((char)('a' + i)).append(evenLetters[i]);
			}
